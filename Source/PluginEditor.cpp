...
DelayEditor::DelayEditor(DelayProcessor& p, juce::AudioProcessorValueTreeState& vts)
    ...
    , apvts_(vts)
{
    delay_gain_slider_attachment_ =
        std::make_unique<SliderAttachment>(apvts_, "delay_gain", delay_gain_slider_);
    addAndMakeVisible(delay_gain_slider_);

    delay_time_slider_attachment_ =
        std::make_unique<SliderAttachment>(apvts_, "delay_time", delay_time_slider_);
    addAndMakeVisible(delay_time_slider_);

    feedback_delay_gain_slider_attachment_ = std::make_unique<SliderAttachment>(
        apvts_, "feedback_delay_gain", feedback_delay_gain_slider_);
    addAndMakeVisible(feedback_delay_gain_slider_);
...
void DelayEditor::resized()
{
    ...
    feedback_delay_gain_slider_.setBounds(bounds.removeFromLeft(100));
}
